...
const LOGO_SRC = "data:image/png;base64,LOGO_BASE64_PLACEHOLDER";
...
const DEFAULT_PRODUCTS = [
  { id: "p1", sku: "FN-DFR-060", name: "DFras", tagline: "Daily Multivitamin & Mineral Complex", category: "Multivitamin", price: 799, cost: 420, stock: 140, reorderLevel: 40, icon: "Leaf", gradient: ["#2F5233", "#7CB342"], image: null },
  { id: "p2", sku: "FN-FRC-060", name: "FrasC", tagline: "Vitamin C 1000mg with Bioflavonoids", category: "Immunity", price: 549, cost: 260, stock: 95, reorderLevel: 30, icon: "Sun", gradient: ["#C9962B", "#F2C879"], image: null },
  { id: "p3", sku: "FN-F12-060", name: "Fras12", tagline: "B-Complex + B12 Energy Formula", category: "Energy & Metabolism", price: 699, cost: 340, stock: 60, reorderLevel: 35, icon: "Zap", gradient: ["#28497D", "#5B8AC9"], image: null },
  { id: "p4", sku: "FN-FB12-030", name: "FrasB12", tagline: "Methylcobalamin B12 Sublingual Drops", category: "Vitamin B12", price: 449, cost: 190, stock: 22, reorderLevel: 30, icon: "Droplet", gradient: ["#1F4E8C", "#6FA8DC"], image: null },
  { id: "p5", sku: "FN-DGF-045", name: "digestaFras", tagline: "Digestive Enzyme Complex", category: "Digestive Health", price: 649, cost: 310, stock: 78, reorderLevel: 25, icon: "FlaskConical", gradient: ["#0F6B5C", "#5FBBA6"], image: null },
  { id: "p6", sku: "FN-FGL-060", name: "FrasGlow", tagline: "Skin Glow Collagen & Biotin", category: "Beauty & Skin", price: 899, cost: 480, stock: 8, reorderLevel: 25, icon: "Sparkles", gradient: ["#B4472F", "#E4907A"], image: null },
  { id: "p7", sku: "FN-RJV-060", name: "Rejuv", tagline: "Joint & Recovery Rejuvenation Formula", category: "Joint & Recovery", price: 1199, cost: 620, stock: 54, reorderLevel: 20, icon: "HeartPulse", gradient: ["#1F3D2B", "#C9962B"], image: null },
];
...
function Sidebar({ tab, setTab, adminUnlocked }) {
  return (
    <aside className="fn-sidebar">
      <div className="fn-brand">
        <img src={LOGO_SRC} alt="FRAS Nutrayu" />
        <div>
          <div className="fn-brand-name">FRAS Nutrayu</div>
      ...
      <div className="fn-nav-note">Data syncs to this artifact's shared storage — visible on every device that opens it.</div>
    </aside>
  );
}
...
function Inventory({ products, openProduct }) {
...
function ProductForm({ initial, onSave, onCancel }) {
  const blank = { id: "", sku: "", name: "", tagline: "", category: CATEGORY_LIST[0], price: "", cost: "", stock: "", reorderLevel: "", icon: "Leaf", gradient: ["#2F5233", "#7CB342"], image: null };
...
function Admin({ products, unlocked, onUnlock, pin, setPin, onAdd, onEdit, onDelete, onResetDemo }) {
  ...
  return (
    <div className="fn-page">
      ...
      <div className="fn-admin-grid">
        ...
        <div className="fn-card">
          <h3 className="fn-h2" style={{ fontSize: 15, marginBottom: 12 }}>Demo data</h3>
          <p style={{ fontSize: 12.5, color: "rgba(31,61,43,.55)", marginBottom: 12 }}>Restore the seven starter products and sample sales history.</p>
          <button onClick={onResetDemo} className="fn-btn outline auto-w" style={{ paddingLeft: 16, paddingRight: 16 }}><RotateCcw size={14} /> Reset to demo data</button>
        </div>
      </div>

      {formOpen && (
        <ProductForm initial={editing} onCancel={() => setFormOpen(false)} onSave={(p) => { (editing ? onEdit : onAdd)(p); setFormOpen(false); }} />
      )}
    </div>
  );
}
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      ...
      let s = null;
      let pn = "1234";
      try { const r = await window.storage.get("fras_products", true); if (r?.value) p = JSON.parse(r.value); } catch (e) {}
      try { const r = await window.storage.get("fras_sales", true); if (r?.value) s = JSON.parse(r.value); } catch (e) {}
      try { const r = await window.storage.get("fras_pin", true); if (r?.value) pn = r.value; } catch (e) {}

      setProducts(p);
      const finalSales = s || genDemoSales(p);
      setSales(finalSales);
      setPinState(pn);

      try { await window.storage.set("fras_products", JSON.stringify(p), true); } catch (e) {}
      if (!s) { try { await window.storage.set("fras_sales", JSON.stringify(finalSales), true); } catch (e) {} }
      try { await window.storage.set("fras_pin", pn, true); } catch (e) {}
  ...
  const persistProducts = useCallback(async (next) => {
    setProducts(next);
    try { await window.storage.set("fras_products", JSON.stringify(next), true); } catch (e) {}
  }, []);

  const persistSales = useCallback(async (next) => {
    setSales(next);
    try { await window.storage.set("fras_sales", JSON.stringify(next), true); } catch (e) {}
  }, []);

  const setPin = useCallback(async (p) => {
    setPinState(p);
    try { await window.storage.set("fras_pin", p, true); } catch (e) {}
  ...
  const handleResetDemo = async () => {
    if (!window.confirm("Reset to the seven demo products and sample sales? This replaces current data.")) return;
    const demoSales = genDemoSales(DEFAULT_PRODUCTS);
    await persistProducts(DEFAULT_PRODUCTS);
    await persistSales(demoSales);
  ...
  return (
    <div className="fn-app">
      <GlobalStyle />
      <Sidebar tab={tab} setTab={setTab} adminUnlocked={adminUnlocked} />

      <div className="fn-main">
        <TopBar tab={tab} />

        {tab === "dashboard" && <Dashboard products={products} sales={sales} openProduct={setSelected} setTab={setTab} />}
        {tab === "inventory" && <Inventory products={products} openProduct={setSelected} />}
      ...
      {editTarget && (
        <ProductForm initial={editTarget} onCancel={() => setEditTarget(null)} onSave={(p) => { handleEditProduct(p); setEditTarget(null); }} />
      )}
